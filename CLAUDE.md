...
**Cards del header Hoy — totales generales + navegación (Sprint 7c-4):**
"Cobrado este mes" → "Cobrado": `App.tsx` usa `sumPaymentsCents(payments)` (`src/domain/paymentsFilter.ts`) en vez de `collectedThisMonth` — total histórico, sin filtro de mes (`collections.ts` queda sin usos pero no se borró). Los otros 3 cards (Capital en la calle, Interés por cobrar, Préstamos activos) ya eran totales generales, verificado sin cambios de lógica. `Stat` (subcomponente de `App.tsx`) gana un `onClick` opcional — con clase `pf-stat-clickable` (cursor pointer + `:active{transform:scale(.97)}`) cuando está presente. Navegación: "Interés por cobrar" y "Préstamos activos" → `setTab("loans")`; "Cobrado" → `setTab("cobros")`; "Capital en la calle" sin `onClick`, no navega. `.gitattributes` (`* text=auto eol=lf`) commiteado en este sprint para normalizar line endings entre máquinas.

**Renovaciones flexibles — modelo préstamo-nuevo (Sprint 7d-1):**
Renovar ya NO muta el préstamo in-place: `renewLoan()` (`src/domain/loanRenewal.ts`, puro, con tests) cierra el préstamo anterior (`isPaid: true`, términos intactos) y crea un préstamo **nuevo** con capital/interés/plazo definidos a mano por Fla, `disbursedAt = vencimiento del anterior` (sin cambio), `renewalCount = anterior + 1` (= número de ciclo) y `renewedFromLoanId` apuntando al anterior (campo nuevo: Dexie v8 + **migración `0011_loans_renewed_from.sql`** + `mappers.ts`). El monto recibido (cualquiera, incluido 0; pre-llenado con el interés del ciclo) se registra como pago tipo `"interest"` ("Renovación") sobre el préstamo **anterior** vía `paymentsRepo.create()` en la misma transacción (`loansRepo.renew()`); si es 0 no hay pago. El capital nuevo NO se deriva de lo recibido. `applyPayment({type:"interest"})` (in-place) se conserva solo para reproducir historial legado — la UI ya no lo genera (segmento "Solo interés" → "Renovar" en `PaymentSheet`, 4 campos + preview). **Distinguir legado vs. nuevo:** `isClosingRenewalPayment()` (pago `interest` último de un préstamo `isPaid` o con hijo `renewedFromLoanId`) — lo usan `balanceCentsAfterPayment` (no rebobina fecha, saldo 0) y `paymentsRepo.cancel` (no corre fecha ni `renewalCount`). **Deshacer:** anular el préstamo nuevo → `loansRepo.cancel` reabre el anterior (`isPaid: false`); recién después se puede anular el pago de renovación (`paymentsRepo.cancel` lo bloquea mientras el hijo esté activo). Comprobante de renovación (inmediato y re-descarga) muestra el préstamo **nuevo** y su saldo. Préstamos cerrados por renovación se etiquetan "Renovado" (no "Pagado") en `ClientDetailSheet`/`LoanCard`. Inputs de interés (soles + %) y plazo (número + presets) extraídos a `src/components/loan/InterestAmountInput.tsx` y `LoanTermInput.tsx`, compartidos por `NewLoanSheet`, `EditLoanSheet` y `PaymentSheet`. No se tocó: creación de préstamos, clasificación de clientes, `loanRules.ts`.
