...
### Hotfix diagnóstico: error al descargar Historial PDF (caso ELVIA PÉREZ)

- **Reporte:** para un cliente con un préstamo + pago creados y luego anulados (soft delete de ambos), la descarga de "Historial" fallaba; otros clientes funcionaban bien.
- **Investigación:** se reprodujo el escenario exacto (préstamo anulado + pago anulado en cascada, cliente sin préstamos activos, nombre con tilde "ÉLVIA PÉREZ") con datos sintéticos contra `ClientHistoryPdf`, usando tanto `renderToBuffer` como el mismo `pdf(...).toBlob()` que usa la app en producción. **No se logró reproducir ningún crash** — el componente ya filtra correctamente por `cancelledAt` (no por un campo `deleted_at` inexistente en este dominio) y maneja bien el caso de cero préstamos activos, `rating` indefinido y caracteres acentuados.
- **Causa raíz sin confirmar:** sin acceso al navegador/datos reales de Fla en este entorno, no se pudo capturar el stack trace real. Los tres handlers de descarga de PDF en `ClientDetailSheet` (`handleDownloadStatement`, `handleDownloadHistory`, `handleDownloadReceipt`) atrapaban el error con `catch { toast.error(...) }` **sin loguearlo** — así que si el error real vuelve a ocurrir, ni la clienta ni un agente futuro podían verlo en la consola.
- **Cambio aplicado:** se agregó `console.error` en los tres `catch` (ahora `catch (err)`) para que el próximo intento fallido dejando su stack trace real en DevTools. Sin cambios de lógica de negocio ni de filtrado de datos — no había un bug concreto que corregir en el filtrado, solo falta de visibilidad del error real.
- **Próximo paso si vuelve a ocurrir:** pedir a Fla abrir DevTools > Console antes de tocar "Historial" para ELVIA PÉREZ y compartir el mensaje de error ahora visible.
