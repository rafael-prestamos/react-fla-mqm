...
### Hotfix zero-interest-submit-disabled: botón "Registrar préstamo" bloqueaba interés 0

- **Reporte:** en `NewLoanSheet` (modo "Nuevo"), con interés 0 el botón "Registrar préstamo" no hacía nada y no mostraba error.
- **Causa raíz:** el sprint 7b-1 (interés 0 permitido) actualizó `validateLoanInput` y el gate de guardado de `EditLoanSheet`, pero dejó afuera una tercera validación redundante: el `disabled` del botón en `App.tsx:971` seguía en `interestCents <= 0`. Con el botón deshabilitado, `onClick={handleSubmit}` nunca corre — por eso no había ningún error visible, `validateLoanInput` ni se llegaba a ejecutar.
- **Fix:** `interestCents <= 0` → `interestCents < 0` en esa línea, igual que las otras dos validaciones.
- **Sin test automatizado nuevo:** `NewLoanSheet` es un componente interno no exportado de `App.tsx` (como `NewClientSheet`, `LoanRowItem`, etc.); el proyecto no tiene `@testing-library/react`/jsdom para simular interacción de inputs (solo `renderToStaticMarkup` para markup estático, ver `ProfileSheet.test.tsx`). Agregar esa infraestructura solo para este `disabled` de una línea era desproporcionado — se verificó con la suite de dominio completa (166/166, sin cambios en `validateLoanInput` ya cubierto por sus tests de sprint 7b-1) + `tsc`/build limpios. Verificación visual en navegador pendiente (extensión Chrome no disponible en este entorno).

### Sprint 7c-1: Eliminar cliente (hard delete) + anular préstamo desde tab Préstamos + quitar tab Atrasados de Hoy

- **`clientsRepo.remove(id)` — hard delete real, no soft delete:** a diferencia de `loansRepo.cancel`/`paymentsRepo.cancel` (que marcan `cancelledAt` y conservan el registro), este método borra el cliente y su historial completo de la fila de Dexie y encola un `enqueue(entity, id, "delete", null)` por cada registro — `pushOutbox()` ya sabía manejar `op === "delete"` desde que existe el tipo `OutboxOp["op"]` (`"put" | "delete"`), solo no había ningún repo que lo emitiera todavía. Elegido explícitamente por instrucción de Giancarlo: Fla quiere poder re-agregar el mismo DNI después, algo que el patrón soft-delete existente (`cancelledAt`) no permite limpiamente (el DNI seguiría "ocupado" por el registro anulado).
- **Bloqueo por préstamos activos:** `remove()` cuenta préstamos con `!isPaid && !cancelledAt` para ese cliente y lanza si hay alguno — mensaje `"No se puede eliminar un cliente con préstamos activos"`. La UI (`ClientDetailSheet`) deshabilita el botón "Eliminar cliente" con el mismo criterio, calculado sobre el prop `loans` que ya recibe (que a su vez viene de `loansRepo.all()`, que ya excluye anulados — así que en la práctica el chequeo del lado UI es solo `!loan.isPaid`, pero se dejó explícito `!loan.isPaid && !loan.cancelledAt` para no depender de ese detalle de `loansRepo.all()`).
- **Cascada real (no soft):** `remove()` borra con `db.loans.where("clientId").equals(id).toArray()` — a diferencia de `loansRepo.byClient`/`.all()`, esta consulta directa a Dexie sí incluye préstamos anulados, así que el hard delete alcanza también los históricos anulados/pagados, tal como pide el sprint. Los pagos de esos préstamos (`db.payments.where("loanId").anyOf(loanIds)`) se borran igual. Todo corre en una única `db.transaction("rw", ...)` (mismo patrón que `loansRepo.cancel`).
- **Confirmación en dos pasos:** `DeleteClientModal.tsx` (nuevo, mismo patrón visual `.cancel-modal`/`.cancel-actions` que `CancelLoanModal`/`CancelPaymentModal`, pero sin campo de motivo ni "escribe ELIMINAR" — el sprint solo pidió Cancelar/Eliminar). `ClientDetailSheet` cierra el sheet (`onClose()`) tras un delete exitoso, ya que el cliente mostrado ya no existe.
- **Anular préstamo desde tab Préstamos:** `LoanCard` (en `App.tsx`, usado por la tab "Préstamos") gana un botón Anular (ícono `Trash2`, `btn-danger`) que abre el mismo `CancelLoanModal` que ya usa `ClientDetailSheet` — sin duplicar el componente ni su lógica de confirmación por texto "ELIMINAR". Nuevo estado `cancellingLoanFromTab` en `App.tsx`, mismo patrón *controlled-sheet* que `editingLoanFromTab` (sprint 6a-8c). El botón se muestra siempre (activos y pagados), igual que en `ClientDetailSheet` — ahí "Anular" nunca estuvo condicionado a `isActive`, así que se mantuvo la misma restricción (la única regla real es la que ya aplica `loansRepo.cancel`: no se puede anular un préstamo ya anulado, y como `loansRepo.all()` excluye anulados de las listas que alimentan la tab, ese caso nunca llega a mostrarse ahí).
- **Quitar tab "Atrasados" de Hoy:** en la pestaña Hoy no existía un selector de tabs real entre "Vence hoy" y "Atrasados" — eran dos secciones (`pf-sect`) apiladas, ambas siempre visibles. Se eliminó la sección "Atrasados" (heading + lista + empty state "Sin atrasados. 🎉") del body de la tab; queda solo "Por vencer" (si aplica) y "Vence hoy". La constante `overdue` (clasificación por `daysLate > 0`) **no quedó huérfana**: se sigue usando en el mini-stat "Atrasados" del header (`pf-mini`), que el sprint no pidió tocar. No se tocó `computeLatePeriods`, `daysLate` ni ninguna clasificación de dominio — solo se quitó la sección de UI, tal como pide el "NO hacer" del sprint.
- **No se tocó:** ninguna migración SQL (hard delete vía Supabase `delete().eq("id", ...)` en `pushOutbox`, ya existente); `loansRepo.cancel`/`paymentsRepo.cancel` (soft-delete de préstamos/pagos individuales) siguen intactos.

### Sprint 7c-2: Vista de Cobros con filtros y totales

- **Nuevo tab "Cobros"** en la navegación principal (`App.tsx`, junto a Hoy/Préstamos/Clientes), ícono `Receipt`. Renderiza `CobrosTab` (`src/components/CobrosTab.tsx`, nuevo), que recibe `clients`/`loans`/`payments` ya cargados por `App.tsx` (mismos arrays que alimentan el resto de tabs, vía `useLiveQuery` sobre los repos existentes — sin query nueva a Dexie). `payments` viene de `paymentsRepo.all()` (excluye cancelados) y `loans` de `loansRepo.all()` (excluye anulados); como anular un préstamo cascada-anula sus pagos (`loansRepo.cancel`), todo pago no-cancelado siempre tiene su préstamo presente en `loans` — no hay lookups huérfanos.
- **Por qué in-memory y no una query Dexie indexada:** se evaluó filtrar directo en Dexie (`db.payments.where("paidAt").between(...)`, aprovechando el índice existente `payments: "id, loanId, paidAt"`) para "no cargar todos los pagos en memoria" como sugería el sprint, pero se descartó — la cartera real es de **~8 clientes**, y el resto de la app (tabs Hoy/Préstamos/Clientes) ya carga el 100% de clients/loans/payments en memoria vía `useLiveQuery` y filtra ahí (ver `activeRows`/`overdue`/`dueToday` en `App.tsx`). Introducir un segundo patrón de acceso a datos solo para esta vista habría sido inconsistente con el resto del código sin beneficio real a esta escala.
- **Filtros puros y testeados en `src/domain/paymentsFilter.ts`** (TDD, `paymentsFilter.test.ts`, 8 tests): `PaymentsDateFilter` (`{ month?, dateFrom?, dateTo? }`), `matchesDateFilter(paidAt, filter)`, `filterPaymentsByDate`, `sumPaymentsCents`. `CobrosTab` mantiene el estado de filtros localmente (no se subió a `App.tsx`, ya bastante grande) y filtra `rows` con `matchesDateFilter` + `clientNameMatches` (mismo helper de búsqueda que Clientes/Préstamos) vía `useMemo`.
- **`paidAt` (timestamp completo) vs. límites de filtro (date-only):** `matchesDateFilter` parsea `paidAt` con `new Date(paidAt)` directo — es un timestamp completo con hora/zona (`nowIso()`), no un date-only string, así que **no** aplica el bug de zona horaria documentado para `disbursedAt`/`lastCycleStart` (ver Sprint 7b-2 arriba); de hecho `collectedThisMonth` (`src/domain/collections.ts`, preexistente) ya usa el mismo patrón `new Date(p.paidAt).getFullYear()/getMonth()`. Los límites del filtro (`month`, `dateFrom`, `dateTo`) sí son date-only (vienen de `<input type="month">`/`type="date"`) y se parsean con `parseLocalDate` para construir los límites de comparación en hora local — ahí sí aplica la regla del sprint de nunca usar `new Date(dateOnlyString)` directo.
- **Mes e intervalo mutuamente excluyentes:** la UI (no el dominio) fuerza la exclusión — `handleMonthChange` limpia `dateFrom`/`dateTo` al setear un mes, `handleRangeChange` limpia `month` al tocar cualquiera de los dos date-pickers. `matchesDateFilter` en sí solo prioriza `month` si por alguna razón llegaran ambos, pero en la práctica la UI nunca los envía juntos.
- **Total y "Limpiar filtros":** card `.pf-stat` destacado arriba de la lista con `sumPaymentsCents(filteredRows)`, recalculado en cada render vía el mismo `useMemo` que filtra las filas — reactivo a cualquier cambio de filtro. Botón "Limpiar filtros" solo aparece si hay algún filtro activo (mes, rango o búsqueda) y resetea los cuatro estados a `""`.
- **Orden y formato de fila:** más reciente primero (`new Date(paidAt).getTime()` descendente, mismo patrón que el historial de pagos en `ClientDetailSheet`). Cada fila muestra cliente, `formatShort(paidAt)`, `paymentMethodLabel` (reutilizado de `src/pdf/formatters.ts` — no tiene import de `@react-pdf/renderer`, así que reusarlo estático no engorda el bundle inicial; ya hay precedente en `PaymentSheet.tsx`), préstamo (`formatSoles(principalCents)` + `formatRatePercent(rate)`) y el monto cobrado.
- **No se tocó:** ninguna regla de dominio de préstamos/pagos, ni `sync`/`outbox`, ni migración SQL. El PDF de este reporte queda explícitamente fuera (sprint 7c-3).

### Ajuste payments-filter-modal: filtro de fecha de Cobros movido a modal

- **Motivo:** los tres controles de fecha inline (mes + desde/hasta) ocupaban mucho espacio vertical en la tab Cobros antes de llegar a la lista; se pidió moverlos a un modal aparte, dejando solo la búsqueda por cliente inline.
- **`PaymentsFilterModal.tsx` (nuevo, `src/components/`):** mismo patrón `.ovl`/`.sheet` + `<h3>Título <span className="x">` que `EditLoanSheet`/`EditClientSheet` (no el patrón de header navy full-screen de `SettingsSheet`, que es para formularios más largos). Recibe el `PaymentsDateFilter` ya aplicado como prop y mantiene su propio estado "draft" (`month`/`dateFrom`/`dateTo`) inicializado desde ese filtro — así, si se reabre el modal, muestra lo que ya estaba aplicado, no un formulario vacío. La exclusión mutua mes↔intervalo es la misma lógica que antes vivía en `CobrosTab` (`handleMonthChange`/`handleRangeChange`), solo que ahora corre sobre el estado draft del modal.
- **Dos acciones, dos semánticas distintas:** "Aplicar" (`btn-p btn-block`) llama a `onApply(draftFilter)` — construye el `PaymentsDateFilter` final (mes si está seteado, si no el intervalo) y cierra el modal; "Limpiar filtros" (`btn` con estilo `--card`) resetea el draft local a vacío, llama a `onClear()` (que en `CobrosTab` hace `setDateFilter({})`) y también cierra — no hace falta un segundo paso porque limpiar ya es una acción completa en sí misma.
- **Botón de filtro con indicador:** en el header de `CobrosTab`, junto al título "Cobros", un botón ícono (`SlidersHorizontal`, mismo estilo `btn` que otros botones-ícono como "Editar cliente") abre el modal. Punto indicador absolutamente posicionado (`var(--accent)`, el caramelo del tema) se muestra solo cuando `hasDateFilter` (`month || dateFrom || dateTo`) es true — la búsqueda por cliente, que sigue siendo un input inline debajo del título (sin tocar), no forma parte de este indicador porque el sprint la dejó explícitamente fuera del modal.
- **`CobrosTab` simplificado:** los cuatro `useState` sueltos (`month`, `dateFrom`, `dateTo`, `clientSearch`) se redujeron a `dateFilter: PaymentsDateFilter` (un solo objeto, seteado por el modal) + `clientSearch` (que se queda igual, inline). `filteredRows` sigue usando `matchesDateFilter(paidAt, dateFilter)` de `src/domain/paymentsFilter.ts` — **sin ningún cambio** en ese archivo ni en sus tests, tal como pedía el ajuste.
- **No se tocó:** el card de "Total cobrado" (sigue arriba de la lista, recalculado igual vía `sumPaymentsCents`), el orden/formato de las filas, ni la posición de la búsqueda por cliente (sigue inline, ahora es el único control de filtro fuera del modal).

### Sprint 7c-3: PDFs de Cobros y de Préstamos activos

- **Dos módulos de dominio puros nuevos, con tests, en vez de lógica dentro de los componentes PDF:**
  - `src/domain/paymentsReport.ts` — `buildPaymentReportRows(clients, loans, payments, filter, clientSearch)` hace el mismo join+filtro que ya vivía inline en `CobrosTab` (payment→loan→client, `matchesDateFilter` + `clientNameMatches`, orden más-reciente-primero). **`CobrosTab` se refactorizó para llamar a esta función** en vez de duplicar la lógica — así el botón "Descargar PDF" genera el reporte con exactamente las mismas filas que la lista ya muestra (única fuente de verdad, sin riesgo de que el PDF y la pantalla se desincronicen). También expone `paymentsReportSubtitle(filter, clientSearch)`, el texto del período ("Agosto 2026" / "16-ago-2026 al 31-ago-2026" / "Todos los cobros", con sufijo `· Cliente: NOMBRE` en mayúsculas si hay búsqueda activa).
  - `src/domain/activeLoansReport.ts` — `buildActiveLoansReport(clients, loans, reference)` filtra `!isPaid && !cancelledAt` (excluye anulados y pagados, tal como pide el sprint), ordena por nombre de cliente y calcula `totalPrincipalCents`/`totalBalanceCents`/`count`. Usado tanto por el botón nuevo en la tab Préstamos como, potencialmente, por cualquier otro consumidor futuro sin duplicar el filtro.
  - Ambos se testean con Vitest puro (sin react-pdf) — exactamente los tres tipos de test que pedía el sprint: "recibe los datos filtrados correctos", "excluye anulados y pagados", "totales calculados correctamente". Los componentes PDF en sí solo reciben las filas/totales ya calculados como props — no repiten ningún filtro.
- **`formatShortDash` (`src/lib/dates.ts`):** nuevo helper "16-ago-2026" (día-mes abreviado-año, con guiones, sin punto final) para el subtítulo de período del PDF de Cobros. No se usó `formatShort` (que da "16 ago." vía `Intl`, con punto y sin año) ni `Intl` en general — se construyó manualmente con un array estático de abreviaturas en español, para tener control exacto del formato pedido y no depender de que el locale del navegador/CI produzca el mismo string. Como en todo el proyecto, las fechas date-only del filtro (`dateFrom`/`dateTo`) se parsean con `parseLocalDate`, nunca `new Date(string)` directo.
- **`CobrosPdf.tsx` (nuevo, `src/pdf/`):** mismo patrón que `GlobalReportPdf.tsx`/`ClientHistoryPdf.tsx` (header navy con logo + `logo-pdf.png`, tabla con `tableHeaderRow`/`tableRow`, footer con datos de contacto + `formatDateTime` en `pageFooter` con `fixed`/`render`). Tabla: Fecha, Cliente, Monto, Préstamo (`formatSoles(principalCents)` + `al` + `formatRatePercent(rate)`), Método (`paymentMethodLabel`). Fila final `totalRow` destacada (fondo `--card`-like, texto en navy) con el total. Botón "Descargar PDF" en `CobrosTab` (ícono `Download`, junto al de filtro) — dynamic import de `@react-pdf/renderer` + `./pdf/CobrosPdf`, mismo patrón try/catch/finally + toast que `handleDownloadGlobalReport` en `App.tsx`. Nombre de archivo (`cobrosPdfFilename`, local a `CobrosTab.tsx`, no exportado — no pedía test explícito) codifica el filtro activo: `cobros-2026-08.pdf` (mes), `cobros-2026-08-16_2026-08-31.pdf` (intervalo), `cobros-todos-<hoy>.pdf` (sin filtro), con sufijo `_<slug-busqueda>` si hay búsqueda por cliente.
- **`ActiveLoansPdf.tsx` (nuevo, `src/pdf/`):** mismo patrón visual. Tabla: Cliente, Capital, Tasa, "Entrega → Vencimiento" (`derived.disbursedDate`/`derived.dueDate` vía `formatDate(date.toISOString())`, mismo patrón que el resto de los PDFs para convertir un `Date` ya derivado a texto), Saldo, Estado. **Estado simplificado a binario "Al día"/"Atrasado"** (`derived.daysLate > 0`) — a propósito más simple que los 6 estados de `STATUS_STYLE` en `App.tsx` (activo/por-vencer/vence-hoy/tolerancia/interés-extra/pagado), porque el sprint pidió explícitamente solo esas dos etiquetas para el PDF; no se tocó `LoanStatus` ni `deriveLoan`. Fila de totales (`totalsBox`, mismo estilo `summaryBox` que `GlobalReportPdf`) con préstamos activos / capital total / saldo total. Botón "Descargar PDF" en la tab Préstamos de `App.tsx` (junto a "+ Nuevo"), mismo patrón dynamic-import + try/catch/finally + toast que `handleDownloadGlobalReport`; `buildActiveLoansReport` se importa estático (es dominio puro, no arrastra `@react-pdf/renderer`) mientras que el componente PDF y la librería sí van en el `Promise.all` dinámico.
- **Regla del bug de sprint 7a-3 respetada:** ningún `<Text>` nuevo usa `fontStyle: "italic"` en ninguno de los dos PDFs — se verificó explícitamente al escribirlos, dado el crash documentado arriba (`Could not resolve font for Helvetica-Bold, fontWeight 400, fontStyle italic`).
- **Tests de humo con render real:** `CobrosPdf.test.ts` y `ActiveLoansPdf.test.ts` siguen el patrón de `ClientHistoryPdf.test.ts` (`pdf(Component(props)).toBlob()`, `expect(blob.size).toBeGreaterThan(0)`) — uno con filas y otro con el estado vacío ("No hay cobros para este filtro." / "No hay préstamos activos."), como red de seguridad adicional ante cualquier combinación de estilos que `react-pdf` no pueda resolver (igual que atrapó el bug de `fontStyle: italic`).
- **No se tocó:** ninguna regla de dominio de préstamos/pagos existente (`loanRules.ts`, `paymentsFilter.ts`), `sync`/`outbox`, ni migración SQL.

### Sprint 7c-4: Cards del header Hoy — totales generales + navegación

- **"Cobrado este mes" → "Cobrado" (total histórico):** `App.tsx` cambia `collectedThisMonth(payments)` (filtraba por mes calendario de la referencia) por `sumPaymentsCents(payments)` (`src/domain/paymentsFilter.ts`, ya existente desde sprint 7c-2, ya testeado). `payments` es el array completo que carga `App.tsx` vía `paymentsRepo.all()` (todo el histórico, sin filtro de fecha) — el cambio es puramente de qué función se llama, no de qué datos se pasan. Se agregó un test más a `sumPaymentsCents` (`paymentsFilter.test.ts`) con pagos en meses/años distintos para dejar explícito que no hay ningún filtro de fecha involucrado, tal como pedía el sprint. `collectedThisMonth`/`src/domain/collections.ts` se dejaron intactos (no se borraron) aunque quedaron sin otros usos en el código — es una función pura pequeña y ya testeada; borrarla no era parte del pedido y no aporta valor a este sprint.
- **Los otros 3 cards ya eran totales generales, verificado sin cambios:** `capitalOut`/`interestOut` (`App.tsx`) ya sumaban sobre **todo** `activeRows` (préstamos activos, sin filtro de fecha) — no sobre un subconjunto del mes. `activeRows.length` (Préstamos activos) es simplemente la cuenta de esa misma lista. No requirieron ningún cambio.
- **Navegación al tocar cada card:** `Stat` (subcomponente de `App.tsx`) gana un prop opcional `onClick`. Cuando está presente, el `<div className="pf-stat">` recibe la clase adicional `pf-stat-clickable` (`cursor:pointer` + `transform:scale(.97)` en `:active`, mismo patrón que `.btn:active`) y atributos `role="button"`/`tabIndex={0}` para accesibilidad básica. "Capital en la calle" no recibe `onClick` — queda visualmente idéntico pero sin ningún indicador de interacción, tal como pedía el sprint. "Interés por cobrar" y "Préstamos activos" → `setTab("loans")`; "Cobrado" → `setTab("cobros")`. Reutiliza el `tab` state y `setTab` que ya maneja la navegación entre pestañas — no se creó ningún mecanismo de navegación nuevo.
- **`.gitattributes` commiteado:** se agrega `* text=auto eol=lf` al repo (estaba sin trackear en el working tree, agregado por Giancarlo entre sprints) para que Git normalice line endings a LF de forma consistente entre distintas máquinas/SO, evitando el ruido de diffs CRLF↔LF que había estado apareciendo en varios sprints anteriores de esta serie.
- **No se tocó:** los PDFs (`CobrosPdf.tsx`/`ActiveLoansPdf.tsx`) ni los filtros de Cobros (`PaymentsFilterModal.tsx`, `paymentsFilter.ts`) del sprint anterior; ninguna migración SQL.

### Migración 0010: formalizar el ALTER TABLE de `loans_term_days_check`

- **`supabase/migrations/0010_loans_term_days_range.sql` no introduce un cambio nuevo en producción** — documenta en el repo un `ALTER TABLE` que ya se había aplicado a mano en el SQL Editor de Supabase durante la sesión de fixes post-release (ver `docs/HANDOFF.md`), porque la constraint de `loans.term_days` (`check (term_days in (25, 28, 30))`, de `0001_init.sql`, reafirmada en `0004_revert_installments.sql`) nunca se actualizó cuando el frontend pasó a permitir plazo libre 1-365 días en el sprint 6a-4 — préstamos con un plazo fuera de {25,28,30} fallaban al sincronizar a Supabase (se guardaban bien en local).
- **`DROP CONSTRAINT IF EXISTS` antes del `ADD CONSTRAINT`** hace la migración idempotente: correrla contra la base de producción (que ya tiene la constraint 1-365 aplicada a mano) no falla — dropea la constraint ya-correcta y la vuelve a crear con el mismo rango. Necesario porque, a diferencia del resto de migraciones del repo, esta formaliza un estado que ya existe en producción en vez de aplicar un cambio nuevo.
- **Resuelve la deuda documentada** en `docs/HANDOFF.md` de la sesión anterior ("Excepción no capturada como migración") — quien reconstruya la base desde cero ahora parte del rango correcto (1-365), no de la constraint vieja (25/28/30).

### Sprint 7d-1: Renovaciones con monto e importes editables (modelo préstamo-nuevo)

- **Problema:** la renovación legada ("Solo interés") asumía que el cliente pagaba exactamente el interés del ciclo y mutaba el préstamo **in-place** (`renewalCount++`, `disbursedAt = dueDate`, mismo capital/tasa/plazo). Fla necesita recibir cualquier monto (incluido 0) y definir a mano capital, interés y plazo del nuevo ciclo.
- **Decisión de modelo: cada renovación crea un préstamo NUEVO** (`src/domain/loanRenewal.ts`, `renewLoan()` — dominio puro con tests), enlazado al anterior por el campo nuevo `Loan.renewedFromLoanId` (Dexie v8 sin índice nuevo; **migración `0011_loans_renewed_from.sql`**: `renewed_from_loan_id uuid null references loans(id) on delete set null`, mapeado en `mappers.ts`). El préstamo anterior queda cerrado (`isPaid: true`, sin tocar `principalCents`/`rate`/`termDays`/`paidOffCents`); el nuevo arranca en el vencimiento del anterior (`disbursedAt = toLocalIsoDate(derived.dueDate)`, comportamiento histórico que el sprint pidió no cambiar), con `renewalCount = anterior + 1` (ahora significa "número de ciclo en la cadena") y `paidOffCents: 0`. **Por qué no in-place:** con capital/tasa/plazo variables por ciclo, mutar el mismo registro perdía los términos del ciclo anterior y rompía dos cosas que dependen de ellos: la reconstrucción de saldos para re-descargar comprobantes (`loanBalanceHistory.ts`, que rebobina `renewalCount × termDays` y re-aplica los abonos contra el capital actual — podía hasta lanzar "El abono excede el saldo" si el nuevo capital era menor) y la anulación de un pago de renovación (`paymentsRepo.cancel`, que solo sabía restaurar la fecha, no capital/tasa). Con un registro por ciclo cada préstamo tiene términos inmutables y toda la lógica existente por préstamo sigue siendo exacta.
- **El monto recibido es un pago normal:** si `receivedCents > 0`, `loansRepo.renew()` lo registra vía `paymentsRepo.create()` (tipo `"interest"` = "Renovación" en historial/PDFs, `daysLate` del ciclo cerrado) sobre el préstamo **anterior**, en la misma transacción Dexie que cierra el anterior y crea el nuevo (3 entradas en el outbox: loan cerrado, loan nuevo, payment). Aparece en Cobros y suma al total cobrado. Si es 0, no se crea pago. `interestPaidCents`/`principalPaidCents` del `PaymentRecord` se reparten contra el interés del ciclo cerrado (solo informativo, igual que en `applyPayment`).
- **`applyPayment` con `type: "interest"` (in-place) se conserva sin cambios** para reproducir el historial legado (préstamos ya renovados en producción con `renewalCount > 0` y `disbursedAt` corrido). La UI ya no lo genera: el segmento "Solo interés" de `PaymentSheet` pasó a ser **"Renovar"**.
- **Distinguir renovación legada vs. de cierre — `isClosingRenewalPayment(loan, pagosCronológicos, id, hasRenewalChild)`:** un pago `"interest"` que es el último activo de un préstamo `isPaid` (o que tiene un préstamo hijo `renewedFromLoanId`, aunque el hijo esté anulado) es una renovación de cierre del modelo nuevo; en el modelo legado eso nunca ocurre (el último pago de un préstamo pagado siempre es `full`/`partial`). Se usa en: (a) `balanceCentsAfterPayment` — no lo cuenta para rebobinar `disbursedAt` y devuelve saldo 0 para ese pago; (b) `rebuildLoanAfterPaymentCancellation` — no corre la fecha ni toca `renewalCount` (que ahora se decrementa desde `loan.renewalCount` en vez de recontar pagos `interest`, equivalente para datos legados).
- **Deshacer una renovación (orden obligatorio):** anular el préstamo **nuevo** (desde tab Préstamos o `ClientDetailSheet`) → `loansRepo.cancel()` detecta `renewedFromLoanId` y **reabre** el anterior (`isPaid: false`) — el pago de renovación se conserva porque fue dinero recibido. Recién entonces se puede anular ese pago: `paymentsRepo.cancel()` lanza "Anula primero el préstamo renovado" mientras exista un hijo activo. Así nunca quedan dos ciclos activos de la misma cadena.
- **Comprobante de una renovación** (inmediato en `PaymentSheet` y re-descarga en `ClientDetailSheet`): muestra el préstamo **nuevo** (capital/tasa/plazo/vencimiento) y su saldo (`deriveLoan(nuevo).balanceCents`) — es lo que el cliente debe ahora. Solo hay botón de descarga si hubo pago (`receivedCents > 0`). `ClientDetailSheet` consulta `db.loans` directo para encontrar al hijo porque su prop `loans` excluye anulados.
- **UI (`PaymentSheet`, modo "Renovar"):** 4 campos — Monto recibido (pre-llenado con el interés del ciclo actual, incluye mora si el flag estuviera activo; hint "puede ser 0"), Capital del nuevo préstamo (pre-llenado con el anterior), Interés nuevo en soles con % (pre-llenado con el interés del anterior; `formatRatePercent`), Plazo (pre-llenado con el anterior, presets 25/28/30 + libre 1-365). Preview: inicio, vencimiento y nuevo saldo. Botón "Recibir S/ X y renovar" / "Renovar sin cobro". Los inputs de interés y plazo se **extrajeron** a `src/components/loan/InterestAmountInput.tsx` y `LoanTermInput.tsx` y se usan en `NewLoanSheet` (App.tsx), `EditLoanSheet` y `PaymentSheet` — un solo markup para los tres.
- **Etiqueta "Renovado":** en `ClientDetailSheet` y `LoanCard` (sección "Pagados") un préstamo `isPaid` con hijo en la cadena se muestra como "Renovado" (navy) en vez de "Pagado" (verde) para no confundir con un cobro total.
- **Validación (`validateRenewLoanInput`):** recibido ≥ 0 entero, capital > 0, tasa ≥ 0, plazo 1-365 (`isValidLoanTerm`). Vive en dominio y se refleja en el `disabled` del botón (misma lección del hotfix zero-interest: las tres capas alineadas desde el inicio).
- **No se tocó:** creación de préstamos nuevos (`loansRepo.create`, `validateLoanInput`), clasificación de clientes, `loanRules.ts`, el flag de mora, `pull.ts`. Tests: 212 (15 nuevos: `loanRenewal.test.ts` ×11, `loanBalanceHistory.test.ts` ×1, `repositories.test.ts` ×3; `mappers.test.ts` ajustado al campo nuevo).
