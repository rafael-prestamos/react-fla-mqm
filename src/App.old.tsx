// ⚠️ LEGACY — Solo referencia visual para Sprint 4b-UI. NO importar desde ningún lugar.
// Este archivo NO compila contra el modelo actual (cuotas). Se elimina al cerrar 4b-UI.
// @ts-nocheck
...
/* ------------------------------------------------------------------ *
 *  Fla MpM ΓÇö Gestor de Pr├⌐stamos (PWA)
...
/* ---------- etiquetas y colores (UI en espa├▒ol) ---------- */
...
const STATUS_STYLE: Record<LoanStatus, StatusStyle> = {
  active: { label: "Al d├¡a", color: "var(--muted)", bg: "var(--paper)", bar: "#C6CFDD" },
  ...
  lateInterest: { label: "Inter├⌐s extra", color: "var(--bad)", bg: "var(--bad-soft)", bar: "var(--bad)" },
...
export default function App() {
  ...
  async function registerPayment(loanId: string, input: { type: PaymentType; amountCents: number; method: PaymentMethod }): Promise<string | null> {
    ...
    if (!target) return "Pr├⌐stamo no encontrado";
    ...
    try {
      ...
      return e instanceof Error ? e.message : "Ocurri├│ un error al procesar el pago";
    }
  }
  ...
  async function createHistoricalLoan(input: LoanBackfillInput): Promise<string | null> {
    try {
      ...
      return e instanceof Error ? e.message : "Error al registrar el pr├⌐stamo hist├│rico";
    }
  }
      ...
      <div className="pf-shell">
        {sync.status === "offline" && (
          <div className="offbar">
            <WifiOff size={14} /> Sin conexi├│n ┬╖ los cambios se guardan aqu├¡ y se sincronizan luego
          </div>
        )}
        ...
        <div className="pf-head">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {sync.status === "syncing" && (
              <div style={{ background: "var(--warn-soft)", color: "var(--warn)", borderRadius: 9, padding: "4px 8px", fontSize: 11, fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
                <RefreshCw size={13} style={{ animation: "spin 1s linear infinite" }} /> SincronizandoΓÇª
              </div>
            )}
            {sync.status === "offline" && (
          ...
          <div className="pf-hello">Buen d├¡a ┬╖ {formatLong(startOfToday())} ┬╖ 7:00 a.m.</div>
        ...
        <div className="pf-body">
          {tab === "today" && (
            <>
              <div className="pf-stats">
                ...
                <Stat icon={<TrendingUp size={13} />} k="Inter├⌐s por cobrar" v={formatSoles(interestOut)} />
                <Stat icon={<Coins size={13} />} k="Cobrado este mes" v={formatSoles(COLLECTED_THIS_MONTH_CENTS)} />
                <Stat icon={<Users size={13} />} k="Pr├⌐stamos activos" v={String(activeRows.length)} />
              </div>

              {dueSoon.length > 0 && (
                <>
                  <div className="pf-sect"><CalendarClock size={14} /> Por vencer (pr├│ximos 3 d├¡as) <span className="cnt">{dueSoon.length}</span></div>
              ...
              {dueToday.length ? (
                ...
                <div className="empty">Nadie vence hoy. Todo tranquilo. ≡ƒæî</div>
              )}
              ...
              {overdue.length ? (
                ...
                <div className="empty">Sin atrasados. ≡ƒÄë</div>
              )}
            </>
          )}
          ...
          {tab === "loans" && (
            <>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "4px 2px 12px" }}>
                <div style={{ fontSize: 18, fontWeight: 700 }}>Pr├⌐stamos</div>
              ...
              {loans.length === 0 ? (
                <div className="empty" style={{ display: "flex", flexDirection: "column", gap: 14, alignItems: "center" }}>
                  A├║n no tienes pr├⌐stamos activos.
          ...
          {tab === "clients" && (
            <>
              ...
              {clients.length === 0 ? (
                <div className="empty">A├║n no tienes clientes. Agrega el primero para empezar.</div>
              ) : (
                clients.map((c) => {
                  ...
                  <div key={c.id} className="row" style={{ flexDirection: "column", alignItems: "stretch", gap: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
                      ...
                      <div className="who">
                        ...
                        <div className="sub">DNI {c.dni} ┬╖ {theirs.length} pr├⌐stamo{theirs.length !== 1 ? "s" : ""}</div>
                      </div>
                    ...
                    <div className="hist">
                      {theirs.map((r) => (
                        <div className="h" key={r.loan.id}>
                          <span>
                            ...
                            {r.loan.renewalCount > 0 ? ` ┬╖ renov├│ ${r.loan.renewalCount}├ù` : ""}
                          </span>
                      ...
                      <div className="h" style={{ fontWeight: 600, borderTop: "1px dashed var(--line)", marginTop: 4, paddingTop: 6 }}>
                        <span>Total hist├│rico prestado</span><span className="num">{formatSoles(totalLent)}</span>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="nav">
          ...
          <NavBtn on={tab === "loans"} onClick={() => setTab("loans")} icon={<Wallet size={20} />} label="Pr├⌐stamos" />
...
function LoanRowItem({ row, onPay }: { row: LoanRow; onPay: () => void }) {
  ...
  const sub =
    d.daysLate > 0
      ? `${d.daysLate} d├¡a${d.daysLate !== 1 ? "s" : ""} de atraso${d.latePeriods > 0 ? ` ┬╖ +${d.latePeriods} inter├⌐s` : ""}`
      : d.daysLate === 0
        ? "Vence hoy"
  ...
  return (
    <div className="row">
      ...
      <div className="amt">
        ...
        {loan.paidOffCents > 0 && <div className="sm num">abon├│ {formatSoles(loan.paidOffCents)}</div>}
      </div>
    ...
    <div className="row" style={{ flexDirection: "column", alignItems: "stretch", gap: 9 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
        <div className="who">
          ...
          <div className="sub">
            {formatSoles(loan.principalCents)} al {loan.rate * 100}% ┬╖ {loan.termDays} d├¡as
            {loan.renewalCount > 0 ? ` ┬╖ renov├│ ${loan.renewalCount}├ù` : ""}
          </div>
        </div>
        <StatusChip status={d.status} />
      </div>
      <div className="preview" style={{ margin: 0 }}>
        <div className="r"><span>Entrega ΓåÆ Pago</span><span className="num">{formatShort(d.disbursedDate)} ΓåÆ {formatShort(d.dueDate)}</span></div>
        <div className="r"><span>Inter├⌐s ({loan.rate * 100}%)</span><span className="num">{formatSoles(d.interestCents)}</span></div>
        {d.lateInterestCents > 0 && (
          <div className="r" style={{ color: "var(--bad)" }}>
            <span>Inter├⌐s por atraso (├ù{d.latePeriods})</span><span className="num">{formatSoles(d.lateInterestCents)}</span>
          </div>
        )}
        <div className="r tot">
      ...
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="field">
          ...
          <div className="seg">
            ...
            <button className={type === "interest" ? "on" : ""} onClick={() => handleTypeChange("interest")}>Solo inter├⌐s</button>
        ...
        {type === "interest" && (
          <div className="preview">
            <div className="r"><span>Cobra el inter├⌐s</span><span className="num">{formatSoles(d.interestCents)}</span></div>
            <div className="r"><span>Renueva {row.loan.termDays} d├¡as ┬╖ nueva fecha</span>
              <span className="num">{formatShort(addDays(d.dueDate, row.loan.termDays))}</span></div>
          </div>
        )}
        {type === "partial" && (
          <div className="field">
            <label>┬┐Cu├ínto abon├│? (S/)</label>
            <input className="inp num" inputMode="decimal" value={amount} onChange={handleAmountChange} placeholder="0.00" />
          </div>
        )}

        <div className="field">
          <label>┬┐C├│mo pag├│?</label>
          <div className="seg">
...
function NewLoanSheet({ clients, ratingOf, onClose, onOpenNewClient, onSubmit, onSubmitHistorical }: {
  ...
  if (mode === "historical") {
    try {
      ...
      // deriveLoan maneja fechas inv├ílidas o NaN si lastCycleStart no est├í completo devolviendo NaN en calculations.
      // As├¡ que lo validaremos solo si la fecha es m├ís o menos parseable.
  ...
  return (
    <div className="ovl" onClick={onClose}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        <h3>Nuevo pr├⌐stamo <span className="x" onClick={onClose}><X size={17} /></span></h3>
        ...
        {clients.length === 0 ? (
          ...
          <>
            <div className="seg" style={{ marginBottom: 16 }}>
              ...
              <button className={mode === "historical" ? "on" : ""} onClick={() => { setMode("historical"); setErrors({}); setSubmitError(null); }}>Registro hist├│rico</button>
            </div>
            ...
            <div className="field">
              ...
              <select className="inp" value={clientId} onChange={(e) => setClientId(e.target.value)}>
                ...
                {clients.map((c) => <option key={c.id} value={c.id}>{c.name} ΓÇö DNI {c.dni}</option>)}
              </select>
            ...
            {isBad && (
              <div className="warn">
                ...
                <span><b>Este cliente qued├│ como mal pagador.</b> Puedes prestarle igual ΓÇö t├║ decides ΓÇö pero revisa su historial antes.</span>
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 11 }}>
              ...
              <div className="field" style={{ flex: 2 }}>
                ...
                <div className="seg">
                  {terms.map((day) => (
                    <button key={day} className={termDays === day ? "on" : ""} onClick={() => setTermDays(day)}>{day} d├¡as</button>
                  ))}
                </div>
            ...
            {mode === "historical" && (
              <>
                <div className="field">
                  <label>Fecha de ├║ltima renovaci├│n (o entrega)</label>
            ...
            {mode === "new" ? (
              <div className="preview">
                ...
                <div className="r"><span>Inter├⌐s ({ratePct || 0}%)</span><span className="num">{formatSoles(interestCents)}</span></div>
                <div className="r"><span>Fecha de pago</span><span className="num">{formatShort(addDays(startOfToday(), termDays))}</span></div>
                <div className="r tot"><span>Deber├í pagar</span><span className="num">{formatSoles(totalCents)}</span></div>
              </div>
            ) : (
              <div className="preview">
                <div className="r"><span>Ciclo actual vence</span><span className="num">{historicalPreview ? formatShort(historicalPreview.dueDate) : "ΓÇö"}</span></div>
                <div className="r"><span>Deuda del ciclo</span><span className="num">{historicalPreview && !isNaN(historicalPreview.debtCents) ? formatSoles(historicalPreview.debtCents) : "ΓÇö"}</span></div>
                ...
                <div className="r tot"><span>Estado</span><span>{historicalPreview ? (
                  historicalPreview.status === "lateInterest" ? "Inter├⌐s extra" :
                  historicalPreview.status === "grace" ? "En tolerancia" :
                  historicalPreview.status === "dueToday" ? "Vence hoy" : "Al d├¡a"
                ) : "ΓÇö"}</span></div>
              </div>
            )}
            ...
            <button
              ...
              onClick={handleSubmit}
            >
              {mode === "new" ? "Registrar pr├⌐stamo" : "Registrar pr├⌐stamo existente"}
            </button>
            {submitError && (
              <div style={{ color: "var(--bad)", fontSize: 13, marginTop: 12, textAlign: "center", fontWeight: 500 }}>
                {submitError}
  ...
  return (
    <div className="ovl" onClick={onClose}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="field">
          <label>DNI</label>
          <input className="inp num" inputMode="numeric" maxLength={8} value={dni} onChange={(e) => setDni(e.target.value)} placeholder="8 d├¡gitos" />
          {errors.dni && <div style={{ color: "var(--bad)", fontSize: 11, marginTop: 4 }}>{errors.dni}</div>}
        </div>

        <div className="field">
          <label>Celular</label>
          <input className="inp num" inputMode="numeric" maxLength={9} value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="9 d├¡gitos" />
